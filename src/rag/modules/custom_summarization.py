...
def _preprocess_messages(
    *,
    ...
    # Calculate the index of the last message that can be summarized
    # (excluding the last keep_last_n_messages messages)
    max_summarizable_index = max(0, len(messages) - keep_last_n_messages)
    ...
    for i in range(total_summarized_messages, max_summarizable_index):
        ...
        # Check if we've reached max_tokens_before_summary
        # We summarize once we have enough tokens, even if remaining messages
        ...
        Args:
            ...
            max_summary_tokens: Maximum number of tokens to budget for the summary.
                ...
                !!! Note
                    ...
                    If you want to enforce it, you would need to pass `model.bind(max_tokens=max_summary_tokens)`
                    as the `model` parameter to this function.
            keep_last_n_messages: Number of most recent messages to always keep unsummarized.
                These messages will never be included in the summary, ensuring recent context is preserved.
                Defaults to 0 (no messages are guaranteed to be kept).
            token_counter: Function to count tokens in a message. Defaults to approximate counting.
                For more accurate counts you can use `model.get_num_tokens_from_messages`.
            initial_summary_prompt: Prompt template for generating the first summary.
            existing_summary_prompt: Prompt template for updating an existing (running) summary.

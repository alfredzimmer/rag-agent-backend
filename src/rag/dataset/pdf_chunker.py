...
def split_pdf(file: str, config_name: str = "ieee", md_output_dir: str = None) -> list[Document]:
    """
    ...
    Args:
        file: Path to the PDF file
        config_name: Name of the configuration to use (e.g., "ieee", "nfpa")
        md_output_dir: Optional directory to save the markdown file. If None, saves to 'src/rag/outputs'.
    ...
    # Save markdown file
    if md_output_dir:
        md_output_path = pathlib.Path(md_output_dir)
    else:
        # Default back to src/rag/outputs if not specified
        md_output_path = pathlib.Path("/srv/shared-data/training-datasets/rag-data/pdf-parsed-md")
        
    md_output_file = md_output_path / f"{name_without_ext}.md"
    md_output_file.parent.mkdir(parents=True, exist_ok=True)
    md_output_file.write_bytes(markdown.encode())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Split a PDF folder into chunks with size requirements.")
    parser.add_argument("--input", type=str, required=True, help="Path to the PDF folder.")
    ...
    parser.add_argument("--md-output", type=str, default="/srv/shared-data/training-datasets/rag-data/pdf-md", help="Path to the output Markdown folder (optional).")
    parser.add_argument("--config", type=str, required=True, help="Header config to use (e.g., 'ieee', 'nfpa')")
    ...
    if MD_OUTPUT_PATH:
        pathlib.Path(MD_OUTPUT_PATH).mkdir(parents=True, exist_ok=True)
    
    for filename in os.listdir(INPUT_PATH):
        if filename.endswith(".pdf"):
            FILE_PATH = os.path.join(INPUT_PATH, filename)
            print(f"Processing {FILE_PATH} with config: {args.config}")
            chunks = split_pdf(FILE_PATH, args.config, md_output_dir=MD_OUTPUT_PATH)
            
            # Get filename for document name
            name_without_ext = os.path.splitext(filename)[0]
            
            formatted_list = format_splits_as_list(chunks, document_name=name_without_ext)
            ...
            print(f"Saved {len(formatted_list)} chunks to {output_file}")
